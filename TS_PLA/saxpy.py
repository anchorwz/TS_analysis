...
##########parallel#######
        self.window = 6 # temporary hard coded
        self.stride = 3
...
#########################
    ...
    def symbol_frequency(self, x):
        """
	    Calculate the frequency of each letter
        """
        (letters, indices) = self.to_letter_rep(x)
...
#        batch_size = len(window_index)/n_jobs
#        batches = chunk(xs, batch_size)
#        symbols = Parallel(n_jobs)(delayed(joblib_symbolized_window)(self, xs, batch) for batch in batches)
#        for f in tfiles: os.unlink(f)
#        return sum(symbols, [])
#
#
